...
typedef enum {
  ...
  // Sets the maximum size in bytes of a single trace output file. This value
  // should be in the range ``[0, INT64_MAX]``. If the value is set to 0, there
  // is no limit on individual file size. The default is a maximum size of
  // 10,485,760 bytes.
  ...
  // value should be in the range ``[0, INT64_MAX]``. If the value is set to 0,
  // there is no limit on the total size of the files. The default is a maximum
  // size of 104,857,600 bytes. If the default roll size is used, this means
  // that a maximum of 10 trace files will be written at a time.
  // Parameter: (Int) max total size of trace files
  FDB_NET_OPTION_TRACE_MAX_LOGS_SIZE = 32,

  // Sets the 'LogGroup' attribute with the specified value for all events in
  // the trace output files. The default log group is 'default'.
  ...
  // execution.  Only applies to code paths first traversed AFTER this option is
  // changed.
  ...
  // Set the passphrase for encrypted private key. Password should be set before
  // setting the key for the password to be used.
  // Parameter: (String) key passphrase
  FDB_NET_OPTION_TLS_PASSWORD = 54,

  // Disables the multi-version client API and instead uses the local client
  // directly. Must be set before setting up the network.
  // Parameter: Option takes no parameter
  FDB_NET_OPTION_DISABLE_MULTI_VERSION_CLIENT_API = 60,

  // If set, callbacks from external client libraries can be called from threads
  // created by the FoundationDB client library. Otherwise, callbacks will be
  // called from either the thread used to add the callback or the network
  // thread. Setting this option can improve performance when connected using an
  // external client, but may not be safe to use in all environments. Must be
  // set before setting up the network. WARNING: This feature is considered
  ...
  // Searches the specified path for dynamic libraries and adds them to the list
  // of client libraries for use by the multi-version client API. Must be set
  ...
  // Enables debugging feature to perform slow task profiling. Requires trace
  // logging to be enabled. WARNING: this feature is not recommended for use in
  // production.
  // Parameter: Option takes no parameter
...
typedef enum {
  ...
  // performance in very large databases where clients access data in a
  // near-random pattern. Defaults to 100000.
  ...
  // the same machine as this client, for better location-aware load balancing.
  ...
  // the same datacenter as this client, for better location-aware load
  // balancing.
  // Parameter: (String) Hexadecimal ID
...
typedef enum {
  ...
  // committed, but might not be the latest committed in the event of a fault or
  // partition
  // Parameter: Option takes no parameter
  ...
  // conflict range. As a result, other transactions which read the key(s) being
  // modified by the next write will not conflict with this transaction. Care
  // needs to be taken when using this option on a transaction that is shared
  // between multiple threads. When setting this option, write conflict ranges
  // will be disabled on the next write operation, regardless of what thread it
  // is on.
  // Parameter: Option takes no parameter
  ...
  // database at the transaction's read version. This option may provide a small
  // performance benefit for the client, but also disables a number of
  // client-side optimizations which are beneficial for transactions which tend
  // to read and write the same keys within a single transaction.
  // Parameter: Option takes no parameter
  ...
  // Specifies that this transaction should be treated as low priority and that
  // default priority transactions should be processed first. Useful for doing
  // batch work simultaneously with latency-sensitive work
  // Parameter: Option takes no parameter
  FDB_TR_OPTION_PRIORITY_BATCH = 201,

  // This is a write-only transaction which sets the initial configuration. This
  // option is designed for use by database system tools only.
  // Parameter: Option takes no parameter
  ...
  // Enables tracing for this transaction and logs results to the client trace
  // logs. Client trace logging must be enabled to get log output.
  // Parameter: (String) String identifier to be used in the logs when tracing
  // this transaction. The identifier must not exceed 100 characters.
  ...
  // INT_MAX]``. If set to 0, will disable all timeouts. All pending and any
  // future uses of the transaction will throw an exception. The transaction can
  // be used again after it is reset. Like all transaction options, a timeout
  // must be reset after a call to onError. This behavior allows the user to
  // make the timeout dynamic.
  ...
  // transaction options, the retry limit must be reset after a call to onError.
  ...
  // Set the maximum amount of backoff delay incurred in the call to onError if
  // the error is retryable. Defaults to 1000 ms. Valid parameter values are
  // ``[0, INT_MAX]``. Like all transaction options, the maximum retry delay
  // must be reset after a call to onError. If the maximum retry delay is less
  // than the current retry delay of the transaction, then the current retry
  // delay will be clamped to the maximum retry delay.
  ...
  // The transaction can read and write to locked databases, and is resposible
  // for checking that it took the lock.
  // Parameter: Option takes no parameter
  FDB_TR_OPTION_LOCK_AWARE = 700,

  // By default, operations that are performed on a transaction while it is
  // being committed will not only fail themselves, but they will attempt to
  // fail other in-flight operations (such as the commit) as well. This behavior
  // is intended to help developers discover situations where operations could
  // be unintentionally executed after the transaction has been reset. Setting
...
typedef enum {
  ...
  // The default. The client doesn't know how much of the range it is likely to
  // used and wants different performance concerns to be balanced. Only a small
  // portion of data is transferred to the client initially (in order to
  ...
  // more expensive than reading individual rows, to minimize cost if iteration
  // stops early.
  ...
  // get high throughput from the database, so if that is what you need consider
  // the SERIAL StreamingMode.
  FDB_STREAMING_MODE_LARGE = 3,

  // Transfer data in batches large enough that an individual client can get
  // reasonable read bandwidth from the database. If the client stops iteration
  // early, considerable disk and network bandwidth may be wasted.
...
typedef enum {
  ...
  // match the length of ``param``. The integers to be added must be stored in a
  ...
  // the value by prepending the appropriate number of zero bytes to ``param``
  ...
  // database is not present, then ``param`` is stored in the database. If the
  // existing value in the database is shorter than ``param``, it is first
  ...
  // Performs a bitwise ``or`` operation.  If the existing value in the database
  // is not present or shorter than ``param``, it is first extended to the
  // length of ``param`` with zero bytes.  If ``param`` is shorter than the
  ...
  // database is not present or shorter than ``param``, it is first extended to
  // the length of ``param`` with zero bytes.  If ``param`` is shorter than the
  // existing value in the database, the existing value is truncated to match
  // the length of ``param``.
  FDB_MUTATION_TYPE_BIT_XOR = 8,

  // Appends ``param`` to the end of the existing value already in the database
  // at the given key (or creates the key and sets the value to ``param`` if the
  // key is empty). This will only append the value if the final concatenated
  // value size is less than or equal to the maximum value size (i.e., if it
  // fits). WARNING: No error is surfaced back to the user if the final value is
  // too large because the mutation will not be applied until after the
  // transaction has been committed. Therefore, it is only safe to use this
  // mutation type if one can guarantee that one will keep the total value size
  // under the maximum size.
  FDB_MUTATION_TYPE_APPEND_IF_FITS = 9,

  // Performs a little-endian comparison of byte strings. If the existing value
  ...
  // Performs a little-endian comparison of byte strings. If the existing value
  // in the database is not present, then ``param`` is stored in the database.
  // If the existing value in the database is shorter than ``param``, it is
  // first extended to the length of ``param`` with zero bytes.  If ``param`` is
  // shorter than the existing value in the database, the existing value is
  // truncated to match the length of ``param``. The smaller of the two values
  // is then stored in the database.
  FDB_MUTATION_TYPE_MIN = 13,

  // Transforms ``key`` using a versionstamp for the transaction. Sets the
  // transformed key in the database to ``param``. The key is transformed by
  // removing the final four bytes from the key and reading those as a
  // little-Endian 32-bit integer to get a position ``pos``. The 10 bytes of the
  // key from ``pos`` to ``pos + 10`` are replaced with the versionstamp of the
  // transaction used. The first byte of the key is position 0. A versionstamp
  // is a 10 byte, unique, monotonically (but not sequentially) increasing value
  // for each committed transaction. The first 8 bytes are the committed version
  // of the database (serialized in big-Endian order). The last 2 bytes are
  // monotonic in the serialization order for transactions. WARNING: At this
  // time, versionstamps are compatible with the Tuple layer only in the Java
  // and Python bindings. Also, note that prior to API version 520, the offset
  // was computed from only the final two bytes rather than the final four
  // bytes.
  FDB_MUTATION_TYPE_SET_VERSIONSTAMPED_KEY = 14,

  // Transforms ``param`` using a versionstamp for the transaction. Sets the
  // ``key`` given to the transformed ``param``. The parameter is transformed by
  // removing the final four bytes from ``param`` and reading those as a
  // little-Endian 32-bit integer to get a position ``pos``. The 10 bytes of the
  // parameter from ``pos`` to ``pos + 10`` are replaced with the versionstamp
  // of the transaction used. The first byte of the parameter is position 0. A
  // versionstamp is a 10 byte, unique, monotonically (but not sequentially)
  // increasing value for each committed transaction. The first 8 bytes are the
  // committed version of the database (serialized in big-Endian order). The
  // last 2 bytes are monotonic in the serialization order for transactions.
  // WARNING: At this time, versionstamps are compatible with the Tuple layer
  // only in the Java and Python bindings. Also, note that prior to API version
  // 520, the versionstamp was always placed at the beginning of the parameter
  // rather than computing an offset.
  FDB_MUTATION_TYPE_SET_VERSIONSTAMPED_VALUE = 15,

  // Performs lexicographic comparison of byte strings. If the existing value in
  // the database is not present, then ``param`` is stored. Otherwise the
  // smaller of the two values is then stored in the database.
  FDB_MUTATION_TYPE_BYTE_MIN = 16,

  // Performs lexicographic comparison of byte strings. If the existing value in
  // the database is not present, then ``param`` is stored. Otherwise the larger
...
fdb_error_t fdb_add_network_thread_completion_hook(void (*hook)(void *),
                                                   void *hook_parameter);
...
fdb_error_t fdb_future_set_callback(FDBFuture *f, FDBCallback callback,
...
fdb_error_t fdb_future_get_string_array(FDBFuture *f, const char ***out_strings,
                                        int *out_count);
...
fdb_error_t fdb_cluster_set_option(FDBCluster *c, FDBClusterOption option,
                                   uint8_t const *value, int value_length);
...
fdb_error_t fdb_transaction_set_option(FDBTransaction *tr,
...
FDBFuture *fdb_transaction_get_addresses_for_key(FDBTransaction *tr,
                                                 uint8_t const *key_name,
                                                 int key_name_length);

FDBFuture *
fdb_transaction_get_range(FDBTransaction *tr, uint8_t const *begin_key_name,
                          int begin_key_name_length, fdb_bool_t begin_or_equal,
                          int begin_offset, uint8_t const *end_key_name,
                          int end_key_name_length, fdb_bool_t end_or_equal,
                          int end_offset, int limit, int target_bytes,
...
fdb_error_t fdb_select_api_version_impl(int runtime_version,
                                        int header_version);
