...
2. `get_range_split_points` — currently a stub; result keys from `fdb_future_get_string_array` are discarded, returns `None` instead of the split-point key list
...
7. `fdb_future_get_string_array` — decode string array results (needed by items 2 and 5)
