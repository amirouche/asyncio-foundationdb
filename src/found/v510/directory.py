...
from found.v510 import transactional
from found.v510 import pack
from found.v510 import Subspace
...
class Directory(object):
    ...
    # Called by all functions that could operate on this subspace
    # directly (move_to, remove, remove_if_exists, exists) Subclasses
    # can choose to return a different directory layer to use for the
    # operation if path is in fact ()
...
class DirectoryLayer(Directory):
    ...
    def __init__(self, node_subspace=Subspace(rawPrefix=b'\xfe'), content_subspace=Subspace(), allow_manual_prefixes=False):
        ...
        # If specified, new automatically allocated prefixes will all
        # fall within content_subspace
    ...
    def _create_or_open_internal(self, tr, path, layer=None, prefix=None, allow_create=True, allow_open=True):
        ...
        if prefix is None:
            ...
            if len(list(tr.get_range_startswith(prefix, limit=1))) > 0:
                msg = "The database has keys stored at the prefix chosen by the automatic prefix allocator: %r."
                msg = msg % prefix
                raise Exception(msg)

            if not self._is_prefix_free(tr.snapshot, prefix):
                msg = "The directory layer has manually allocated prefixes that conflict with the automatic prefix allocator."
                raise Exception(msg)
    ...
    def move(self, tr, old_path, new_path):
        ...
        if old_path == new_path[:len(old_path)]:
            msg = "The destination directory cannot be a subdirectory of the source directory."
        ...
        if not parent_node.exists():
            msg = "The parent of the destination directory does not exist. Create it first."
            raise ValueError(msg)
    ...
    def _check_version(self, tr, write_access=True):
        ...
        if version[1] > self.VERSION[1] and write_access:
            msg = "Directory with version %d.%d.%d is read-only when opened using directory layer %d.%d.%d"
    ...
    def _is_prefix_free(self, tr, prefix):
        ...
        return prefix and not self._node_containing_key(tr, prefix) \
            and not len(list(tr.get_range(self._node_subspace.pack((prefix,)), self._node_subspace.pack((strinc(prefix),)), limit=1)))

    def _is_prefix_empty(self, tr, prefix):
        return len(list(tr.get_range(prefix, strinc(prefix), limit=1))) == 0
    ...
    if isinstance(path, tuple):
        ...
        for i, name in enumerate(path):
            if isinstance(name, bytes):
                ...
                msg = 'Invalid path: must be a unicode string or a tuple of unicode strings'
                raise ValueError(msg)
        return tuple(path)
